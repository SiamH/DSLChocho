...
import ca.queensu.cs.mase.urml.ChooseStatement;
...
public class StatementExecuter {
	...
	private void compute(ChooseStatement choose, CapsuleContext ctx)
	{
		Map<LocalVar, Value> envt = ctx.callStackOfLocalVars().peek();
		LocalVar lvalue = choose.getVar();
		Value v = ExpressionEvaluator.interpret(choose.getExpression(), ctx);
		int i = ((Int)v).getVal();
		...
		if (i > 0)
		{
			...
			result = randomGenerator.nextInt(i);
			
		}
				
		envt.put(lvalue, new Int(result));
	 }
